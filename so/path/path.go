...
// Windows paths with drive letters or backslashes.
//
// Based on the [path] package.
//
// [path]: https://github.com/golang/go/blob/go1.26.2/src/path
package path
...
// an empty string.
func Join(a mem.Allocator, elem ...string) string {
	size := 0
	for _, e := range elem {
		size += len(e)
	}
	...
	buf := mem.AllocSlice[byte](a, 0, size+len(elem)-1)
	for _, e := range elem {
		if len(buf) > 0 || e != "" {
			if len(buf) > 0 {
				buf = append(buf, '/')
...
// If the allocator is nil, uses the system allocator.
// The returned string is allocated; the caller owns it.
func Dir(a mem.Allocator, path string) string {
	dir, _ := Split(path)
	return Clean(a, dir)
}
...
func (b *lazybuf) append(c byte) {
	if b.buf == nil {
		...
		n := len(b.s)
		b.buf = mem.AllocSlice[byte](b.a, n, n)
		copy(b.buf, b.s[:b.w])
	}
	b.buf[b.w] = c
	b.w++
