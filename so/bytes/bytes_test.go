...
func TestIndexAny(t *testing.T) {
	runIndexAnyTests(t, IndexAny, "IndexAny", indexAnyTests)
}
...
func TestSplitAfter(t *testing.T) {
	for _, tt := range splitaftertests {
		a := SplitAfterN(nil, []byte(tt.s), []byte(tt.sep), tt.n)
		...
		if tt.n < 0 {
			b := sliceOfString(SplitAfter(nil, []byte(tt.s), []byte(tt.sep)))
			if !slices.Equal(result, b) {
...
func TestFieldsFunc(t *testing.T) {
	for _, tt := range fieldstests {
		a := FieldsFunc(nil, []byte(tt.s), unicode.IsSpace)
		result := sliceOfString(a)
		if !slices.Equal(result, tt.a) {
...
func TestToValidUTF8(t *testing.T) {
	for _, tc := range toValidUTF8Tests {
		got := ToValidUTF8(nil, []byte(tc.in), []byte(tc.repl))
...
func TestToTitle(t *testing.T) {
	for _, tt := range ToTitleTests {
		if s := string(ToTitle(nil, []byte(tt.in))); s != tt.out {
